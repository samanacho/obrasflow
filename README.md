...
Control de producción de postes de hormigón bajo especificaciones técnicas de la ANDE — sección global aparte de las obras (sidebar + al lado de Dashboard/Tablero/Tabla), con su propio modelo relacional (`PoleSpec`/`PoleLot`/`PoleQualityTest`/`RawMaterial`/`PoleRecipeItem`/`PoleLotMaterialConsumption` en `prisma/schema.prisma`) en vez del patrón `ProjectItem`/JSON que usan los módulos de proyecto, porque acá los campos son fijos y conocidos de antemano.
...
Cuatro pestañas dentro de `/postes`:
- **Resumen**: especificaciones activas, lotes en proceso/aprobados/rechazados, stock de postes en depósito (aprobados menos despachados), costo total de materia prima consumida (todos los lotes, en guaraníes), donut de lotes por estado.
