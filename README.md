...
Control de producción de postes de hormigón bajo especificaciones técnicas de la ANDE — sección global aparte de las obras (sidebar + al lado de Dashboard/Tablero/Tabla), con su propio modelo relacional (`PoleSpec`/`PoleLot`/`PoleQualityTest`/`RawMaterial`/`PoleRecipeItem`/`PoleLotMaterialConsumption`/`MaterialPurchase` en `prisma/schema.prisma`) en vez del patrón `ProjectItem`/JSON que usan los módulos de proyecto, porque acá los campos son fijos y conocidos de antemano.
...
Cinco pestañas dentro de `/postes`:
- **Resumen**: especificaciones activas, lotes en proceso/aprobados/rechazados, stock de postes en depósito (aprobados menos despachados menos reservados para ensayo), costo total de materia prima consumida (todos los lotes, en guaraníes), donut de lotes por estado.
- **Especificaciones**: catálogo de tipos de poste que produce la fábrica (longitud, esfuerzo nominal, diámetro, calidad del hormigón, armadura, norma ANDE) — CRUD completo; no se puede eliminar una especificación con lotes cargados (se sugiere marcarla inactiva). Cada especificación tiene su propia ficha (`/postes/specs/[id]`) con la **receta de producción** (ver abajo).
- **Materias primas**: catálogo de insumos (cemento, hierro, arena...) con unidad de medida libre y **costo unitario en guaraníes editable en cualquier momento** — así se actualiza según el precio de mercado del día. Muestra consumo histórico, comprado histórico y **stock disponible estimado** (comprado − consumido) por material. No se puede eliminar una materia prima en uso en alguna receta, consumo o compra (se sugiere marcarla inactiva).
- **Compras**: trazabilidad de compras de materia prima — fecha, tipo de documento (factura/orden de compra/remisión/otro), N° de documento, proveedor, cantidad y costo pagado. Es la fuente del stock disponible que se muestra en Materias primas.
- **Lotes de producción**: cada lote (código, especificación, cantidad, fecha de colado/desmolde, responsable, ciudad de destino) recorre `en_curado → listo_para_ensayo → en_ensayo → aprobado/rechazado → despachado`, con filtro por estado. Por defecto un lote es de **101 postes**: 100 a entregar y 1 reservado para el ensayo destructivo de fiscalización (`cantidadParaEnsayo`, editable) — el disponible para despacho descuenta ese reservado además de lo ya despachado. Cada lote tiene su propia ficha (`/postes/lotes/[id]`) con su ID, los datos de aprobación ANDE (fecha, N° de acta, inspector, **numeración ANDE asignada** a los postes entregados), una **estimación de fecha de fiscalización** (colado + 15 días corridos de curado, vía [lib/factoryCapacity.ts](lib/factoryCapacity.ts)) mientras no esté aprobado, el **consumo de materia prima** del lote (ver abajo) y el listado de **ensayos de calidad** (ruptura/flexión, verificación dimensional, curado) con alta/eliminación.

**Receta y consumo de materia prima** (`PoleRecipeItem` / `PoleLotMaterialConsumption`): en la ficha de cada especificación se arma la receta agregando materiales uno por uno con su cantidad por poste — eso da un **costo estimado por poste** en guaraníes, siempre calculado con el costo *actual* de cada material (útil para presupuestar). Al crear un lote, el servidor congela una copia de la receta (cantidad y costo unitario del momento) en `PoleLotMaterialConsumption`, así el costo de un lote ya producido no se mueve después aunque cambie el precio de mercado o se edite la receta; si se edita la cantidad de postes del lote, se re-escala la cantidad/costo total consumido pero con el costo unitario que ya estaba congelado. Todos los montos de este módulo están siempre en guaraníes (`Gs.`, sin decimales — ver [lib/currency.ts](lib/currency.ts)).

**Resumen en Inicio**: la pestaña Dashboard de la página principal (`app/page.tsx`) tiene una card "🏭 Fábrica de Postes" con lotes en proceso, postes en stock, fiscalizaciones ANDE próximas (próximos 7 días, con link a cada lote) y la capacidad de producción de hoy. **Capacidad y calendario de la fábrica** ([lib/factoryCapacity.ts](lib/factoryCapacity.ts)): trabaja de lunes a sábado 7:00–17:00 (sábado hasta las 12:00, domingo cerrado), con una capacidad de 6 postes/día de lunes a viernes y 3 los sábados (33/semana) — son constantes operativas fijas de la fábrica, no datos cargados por el usuario.
